...
#############################################################
    if(isset($_POST['btnregister'])) {
		//check is register
		$check_register = $pdo->pdoExecute("SELECT * FROM `users` WHERE `id_number` = '".$_POST['id_number']."'");
		if($pdo->pdoRowCount($check_register) == 1){
			$msg = 'اسم مسجل لدينا قم بتسجيل الدخول ليتم تسجيلك في البرنامج';	
		} else {
		if($_POST['password'] != $_POST['re-password']){
			...
			$data['institute_id']				= trim($_POST['institute_id']);
			$data['types_diploma_id']			= trim($_POST['diploma_id']);
			...
			$data['program_id']					= $_GET['id'];
			$data['events_programs']			= implode(",",$_POST['events_programs']);
    ...
    <div class="row">
      <div id="sp-component" class="col-sm-12 col-md-12">
        <div class="sp-column ">
          ...
          <div class="row">
          <? if($_GET['do'] == 'program'){  
          echo '<div class="alert alert-warning" role="alert">اذا كنت مسجلاً من قبل قم <a href="login.php">بتسجيل دخولك</a> حتى يتم تسجيلك في البرنامج بشكل مباشر</div>'; } ?>
        ...
        $("input[name='is_work_teaching_qoran']").click(function () {
            if ($("#chkYes").is(":checked")) {
              ...
              <fieldset>
                ...
                <? if($_GET['do'] != 'program' && $_GET['do'] != 'competition'){ ?>
                <!-- Select Basic -->
                <div class="control-group col-md-6">
...
?>
                ...
                <div class="control-group col-md-6">
                  <label class="control-label" for="diploma_id">نوع الدبلوم </label>
                  <div class="controls">
                    <select id="diploma_id" name="diploma_id" class="form-control" required>
                      <option value="0">---أختر الدبلوم---</option>
                      <?
                                    $sql_types_diploma = $pdo->pdoGetAll("SELECT * FROM `diploma` ORDER BY `id` DESC");
...
?>
                ...
                <? } ?>
                <div class="control-group col-md-6">
                  <label class="control-label" for="diploma_id">مكان السكن</label>
                  <div class="controls">
