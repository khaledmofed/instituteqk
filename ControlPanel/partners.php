...
#############################################################
    ...
    if(isset($_POST['btnadd'])) {
		...
		list($file,$error) = upload('image');
            ...
            $resizeObja -> resizeImage(265, 'landscape');		
            $resizeObja -> saveImage($destFile_orignal, 100);
		}
		$data['title']				= trim($_POST['title']);
		$data['img']				= $file;
		$data['url']				= trim($_POST['url']);
		$insert						= $pdo->pdoInsUpd('partners', $data);
...
#############################################################
    ...
    if(isset($_POST['btnedit'])) {
		...
		$data['url']				= trim($_POST['url']);
		$where[id] 					= $_GET['id'];
		$update 					= $pdo->pdoInsUpd('partners', $data, 'update', $where);
		header('Location: ?do=show&process=successfully');
    }
...
<? include('right.php') ?>
    ...
    <?
        ...
        {
        case "show":
        include("template/partners/show.php");
        break;
        case "add":
        include("template/partners/add.php");
        break;
        case "edit":
        include("template/partners/edit.php");
        break;
        }
