...
#############################################################
    ...
    if(isset($_POST['btnadd'])) {
		...
		$data['date_competition']	= trim($_POST['date_competition']);
		$data['time_competition']	= trim($_POST['time_competition']);
		$data['need_attend']		= trim($_POST['need_attend']);
		$data['insert_id']			= $_SESSION['insert_id'];
		$insert						= $pdo->pdoInsUpd('competitions', $data);
...
#############################################################
    ...
    if(isset($_POST['btnedit'])) {
		...
		$data['need_attend']		= trim($_POST['need_attend']);
		$where[id] 						= $_GET['id'];
		$update 						= $pdo->pdoInsUpd('competitions', $data, 'update', $where);
		header('Location: ?do=show&process=successfully');
    }
...
<? include('right.php') ?>
    ...
    <?
        ...
        {
        case "show":
        include("template/competitions/show.php");
        break;
        case "add":
        include("template/competitions/add.php");
        break;
        case "edit":
        include("template/competitions/edit.php");
        break;
        }
