...
#############################################################
    ...
    if(isset($_POST['btnadd'])) {
		...
		$data['block']				= trim($_POST['block']);
		$data['insert_id']			= $_SESSION['insert_id'];
		$insert						= $pdo->pdoInsUpd('footer_menu', $data);
...
#############################################################
    ...
    if($_GET['do'] == 'edit'){
        $sql = "SELECT * FROM `footer_menu` where id=:id";
        $data[id] = $_GET['id'];
...
#############################################################
    ...
    if(isset($_POST['btnedit'])) {
		...
		$data['insert_id']			= $_SESSION['insert_id'];
		$where[id] 						= $_GET['id'];
		$update 						= $pdo->pdoInsUpd('footer_menu', $data, 'update', $where);
		header('Location: ?do=show&process=successfully');
    }
...
#############################################################
	...
	if(isset($_GET['del'])){
        $sql 		= "DELETE FROM `footer_menu` WHERE `id`=:id";
        $data[id] 	= $_GET['del'];
...
<? include('right.php') ?>
    ...
    <?
        ...
        {
        case "show":
        include("template/footer_menu/show.php");
        break;
        case "add":
        include("template/footer_menu/add.php");
        break;
        case "edit":
        include("template/footer_menu/edit.php");
        break;
        }
