...
#############################################################
# Acceptable Register Events
#############################################################
	if(isset($_GET['case_register_events'])){
		$data['case_register']		= 'acceptable';
		$where[id] 					= $_GET['id'];
		$update 					= $pdo->pdoInsUpd('register_events', $data, 'update', $where);
		header('Location: ?do=show&type=program&case_register=wait&process=successfully');
    }
#############################################################
# Acceptable Register Events
#############################################################
	if(isset($_GET['case_register_events'])){
		$data['case_register']		= 'unacceptable';
		$where[id] 					= $_GET['id'];
		$update 					= $pdo->pdoInsUpd('register_events', $data, 'update', $where);
		header('Location: ?do=show&type='.$_GET['type'].'&case_register=wait&process=successfully');
    }
?>
<!DOCTYPE html>
...
<? include('right.php') ?>
    ...
    <?
        ...
        {
        case "show":
        include("template/students/show.php");
        break;
        case "add":
        include("template/students/add.php");
        break;
        case "edit":
        include("template/students/edit.php");
        break;
        }
